...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return db.transaction(async tx => {
    // Serializes last-admin checks, so concurrent edits cannot remove every admin.
    ...
    if (target.role === "admin" && (role !== "admin" || !active)) {
      const [{ value }] = await tx.select({ value: count() }).from(users).where(and(eq(users.role, "admin"), eq(users.active, true), ne(users.id, id)));
    ...
    if (body.password !== undefined) {
      ...
      if (!validNewPassword(password)) return Response.json({ error: "Heslo musí mít 15 až 128 znaků." }, { status: 400 });
      changes.passwordHash = await hashPassword(password);
    }
    const revokeSessions = body.password !== undefined || !active || role !== target.role;
    const [updated] = await tx.update(users).set({ ...changes, ...(revokeSessions ? { sessionVersion: sql`${users.sessionVersion} + 1` } : {}) }).where(eq(users.id, id)).returning({ id: users.id, name: users.name, email: users.email, role: users.role, active: users.active, createdAt: users.createdAt });

...
2. For every changed or added handler under `app/api/v2/**/route.ts`:
   - Confirm it calls the session/role helpers from `lib/auth.ts` before touching data, and that the required role matches the operation (writes need admin or manager; user management needs admin).
...
6. Run `npm run lint` and `npx tsc --noEmit` and include failures as findings.
